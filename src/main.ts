...
function routeLink(path: string, label: string): string {
  ...
  return `<a href="${path}" data-route ${current ? 'aria-current="page"' : ''}>${label}</a>`;
}
...
function header(): string {
  return `
    ...
    <header class="site-header">
      <a class="wordmark" href="/" data-route aria-label="Puzzle Late home"><span aria-hidden="true">◇</span> Puzzle Late</a>
      <nav aria-label="Main navigation">
        ${routeLink('/demo', 'Demo')}
        <a href="#shelf">Puzzles</a>
...
function puzzleControls(puzzle: Puzzle): string {
  if (round.phase === 'won') {
    ...
    return `<div class="round-ending" aria-labelledby="ending-heading">
      ...
      <h3 id="ending-heading">Puzzle complete</h3>
  ...
  if (round.phase === 'lost') {
    return `<div class="round-ending" aria-labelledby="lost-heading">
      ...
      <h3 id="lost-heading">This page folded away</h3>
  ...
  return `<div class="answer-area">
    <p class="attempts" aria-live="polite"><span aria-hidden="true">${'●'.repeat(round.attempts)}${'○'.repeat(2 - round.attempts)}</span> ${round.attempts} mark${round.attempts === 1 ? '' : 's'} left</p>
    <fieldset>
    ...
    ${round.hintsShown > 0 ? `<ol class="nudges">${puzzle.hints.slice(0, round.hintsShown).map((hint) => `<li>${escape(hint)}</li>`).join('')}</ol>` : ''}
  </div>`;
}
...
function gameCard(puzzle: Puzzle): string {
  return `<section class="game-card" aria-labelledby="puzzle-heading" data-phase="${round.phase}">
    ...
    <h2 id="puzzle-heading">${escape(puzzle.title)}</h2>
...
function shelf(): string {
  ...
  return `<section id="shelf" class="shelf" aria-labelledby="shelf-heading">
    <div class="section-heading">
      <div><p class="eyebrow">Today’s featured puzzle</p><h2 id="shelf-heading">Choose a puzzle</h2></div>
...
function offer(): string {
  return `<section class="offer" aria-labelledby="offer-heading">
    ...
    <div><p class="eyebrow">Full anthology</p><h2 id="offer-heading">Get all 40 original puzzles</h2><p>Five puzzles are free. The complete anthology adds 35 timing, shadow, and route deductions with local progress.</p><p><strong>One-time purchase.</strong> Checkout and activation are not registered yet, so buying is not available on this release.</p><button class="secondary-button" type="button" disabled aria-describedby="offer-status">Purchase registration pending</button><p id="offer-status" class="muted">This button is unavailable until the separate billing operator registers the offer.</p></div>
  </section>`;
}

function home(): string {
  const puzzle = activePuzzle();
  setTitle(mode === 'demo' ? 'Demo — Puzzle Late' : 'Puzzle Late — Play original visual puzzles', 'Play original five-minute timing, shadow, and route puzzles in your browser.');
  return `<main id="main" tabindex="-1">
    ${demoBanner()}
    <section class="play-intro" aria-labelledby="page-heading">
      <div class="intro-copy">
        <p class="eyebrow">40 original puzzles</p>
        <h1 id="page-heading" tabindex="-1">Solve original visual puzzles</h1>
        <p class="lede">For daily-puzzle fans who want another short challenge now.</p>
        <div class="intro-actions"><a class="primary-button" href="/demo" data-route>Try it with sample data</a><span>Starts a saved sample run.</span></div>
        <ul class="plain-facts"><li>Five free puzzles.</li><li>No account or tracking.</li><li>Progress stays in this browser.</li></ul>
      </div>
    ...
    <section class="now-playing" aria-labelledby="now-heading"><div class="now-heading"><p class="eyebrow">Play now</p><h2 id="now-heading">${mode === 'demo' ? 'Sample puzzle in progress' : 'Featured puzzle ready'}</h2></div>${gameCard(puzzle)}</section>
    <section class="how-it-works" aria-labelledby="how-heading"><h2 id="how-heading">How to play</h2><ol><li><strong>Read the paper scene.</strong><span>Each puzzle gives the timing, shadow, or route clues you need.</span></li><li><strong>Choose one answer.</strong><span>You have two marks and two optional non-spoiler nudges.</span></li><li><strong>Finish a card.</strong><span>Each solution reveals a small ending and saves local progress.</span></li></ol></section>
    ${shelf()}
    <section class="privacy-summary" aria-labelledby="privacy-heading"><h2 id="privacy-heading">Your progress and privacy</h2><p>Your puzzle progress is stored only in this browser. There are no accounts, ads, analytics, or puzzle submissions.</p><a href="/privacy" data-route>Read the privacy details</a></section>
    ${offer()}
  </main>`;
}
...
function legalPage(kind: 'privacy' | 'terms'): string {
  ...
  return `<main id="main" tabindex="-1" class="legal-page"><article><p class="eyebrow">Puzzle Late</p><h1 tabindex="-1">${privacy ? 'Privacy and local progress' : 'Terms for the puzzle anthology'}</h1>
    ${privacy ? `<h2>What stays on your device</h2><p>Puzzle progress and the optional still-motion setting are saved in your browser’s local storage. Demo progress uses a separate local storage key and is deleted when you choose “Start for real”.</p><h2>What we do not collect</h2><p>This release has no account, analytics, advertising, payment form, or third-party scripts. The game makes no network requests after the page is loaded.</p><h2>Clear your data</h2><p>Use your browser’s site-data controls to remove local progress. Reset demo only resets the isolated sample progress.</p>` : `<h2>Using the free sample</h2><p>The first five original puzzles are free to play in this browser. Your progress is local to this browser and can be cleared through browser site-data controls.</p><h2>The full anthology</h2><p>The complete 40-puzzle anthology is planned as a one-time purchase. This release does not have a registered checkout or license activation path. Do not rely on a purchase button until one is available.</p><h2>Original work</h2><p>Puzzle Late uses original puzzle text, diagrams, and paper-diorama artwork. It does not reproduce or archive any publisher’s puzzles.</p>`}
    <p><a href="/" data-route>Return to the puzzles</a></p></article></main>`;
}

function notFound(): string {
  setTitle('Page not found — Puzzle Late', 'The requested Puzzle Late page was not found.');
  return `<main id="main" tabindex="-1" class="not-found"><section><p class="eyebrow">404</p><h1 tabindex="-1">This puzzle page is missing</h1><p>The page may have moved, or the address may be incomplete.</p><a class="primary-button" href="/" data-route>Play a puzzle</a></section><div class="lost-card" aria-hidden="true">◇<span>?</span></div></main>`;
}

function settingsDialog(): string {
  if (!settingsOpen) return '';
  return `<dialog class="settings-dialog" open aria-labelledby="settings-heading"><form method="dialog">
    <div class="dialog-heading"><h2 id="settings-heading">Game settings</h2><button type="button" class="icon-button" data-action="close-settings" aria-label="Close settings">×</button></div>
    ...
    <button type="button" class="primary-button" data-action="close-settings">Save settings</button>
  </form></dialog>`;
}
...
function render(): void {
  ...
  app.innerHTML = `${header()}${page}${footer()}<div class="sr-status" aria-live="polite" aria-atomic="true">${escape(round.note)}</div>${settingsDialog()}`;
  attachEvents();
  if (settingsOpen) app.querySelector<HTMLButtonElement>('[data-action="close-settings"]')?.focus();
}
...
function choosePuzzle(id: number): void {
  ...
  render();
  app.querySelector<HTMLElement>('#puzzle-heading')?.focus?.();
}
...
function answer(choice: string): void {
  ...
  render();
  app.querySelector<HTMLElement>('.round-ending h3, .prompt')?.focus?.();
}

function navigate(path: string): void {
  const wantedMode: PlayMode = path === '/demo' ? 'demo' : 'real';
  if (path === '/demo' && mode !== 'demo') {
    mode = 'demo';
    progress = loadProgress(mode).progress;
    freshRound('Demo started. Sample progress is separate from your local game.');
  } else if (mode === 'demo' && path === '/') {
    mode = wantedMode;
    abandonDemo();
    progress = loadProgress(mode).progress;
    freshRound('Demo discarded. Your local game is ready.');
  }
...
function attachEvents(): void {
  ...
  app.querySelectorAll<HTMLButtonElement>('[data-action]').forEach((button) => button.addEventListener('click', () => {
    ...
    if (action === 'restart') { freshRound('Puzzle restarted.'); render(); }
    if (action === 'next-puzzle') { choosePuzzle(nextPuzzle(activePuzzle().id, progress.completed).id); }
    if (action === 'choose-free') { choosePuzzle(1); document.querySelector('#shelf')?.scrollIntoView({ behavior: progress.settings.still ? 'auto' : 'smooth' }); }
    if (action === 'reset-demo') { progress = resetProgress('demo'); freshRound('Demo reset to its sample progress.'); render(); }
    if (action === 'start-real') { abandonDemo(); mode = 'real'; progress = loadProgress('real').progress; freshRound('Demo discarded. Your local game is ready.'); history.pushState({}, '', '/'); render(); }
    if (action === 'settings') { settingsOpen = true; render(); }
    if (action === 'close-settings') { settingsOpen = false; persist(); render(); }
  }));
...
window.addEventListener('popstate', () => {
  mode = location.pathname === '/demo' ? 'demo' : 'real';
  progress = loadProgress(mode).progress;
  freshRound();
  render();
  requestAnimationFrame(() => app.querySelector<HTMLElement>('h1')?.focus());
